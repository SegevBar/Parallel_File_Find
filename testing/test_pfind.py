...
def populate_dir_tree(base_path: str, max_depth: int = 6, max_files: int = 10):
    def __populate_dir_tree_rec(base_path: str, depth: int):
        for i in range(randint(1, max_files)):
...
def run_pfind_with_psutil(dirpath: str, term: str, num_threads: int):
    psutil = pytest.importorskip("psutil")
    proc = subprocess.Popen(
        ["./a.out", dirpath, term, str(num_threads)],
    )
    ...
    while proc.poll() is None:
        with p.oneshot():
            max_num_threads = max(max_num_threads, p.num_threads())
            threads = p.threads()
            readings.append(threads)
    return proc, max_num_threads
...
@pytest.mark.parametrize(
    ...
    [
        ...
        for i in [1, 2, 4, 8, 16, 32, 64, 1024]
    ],
)
...
def test_num_threads(dir_tree):
    num_threads = 12
    proc_res, max_num_threads = run_pfind_with_psutil(dir_tree, "freedom", num_threads)
    assert (
        proc_res.returncode == 0
    ), f"pfind returned an error status: {proc_res.stderr}"
    assert max_num_threads == num_threads + 1, (
        f"pfind used {max_num_threads - 1} threads (other than "
        f"the main one) at its peak, while {num_threads} were requested"
    )
